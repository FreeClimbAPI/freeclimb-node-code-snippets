...
const { Say, Pause, Prompt, PerclScript, GetDigits, Hangup } = require('@freeclimb/sdk')
...
app.post('/colorSelectionDone', (req, res) => {
  ...
  if (digits) {
    ...
    const sayResponse = color ? `You selected ${color}` : 'you did not select a number between 1 and 3'
    const say = new Say({ text: sayResponse })
